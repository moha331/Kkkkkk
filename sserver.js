...
const CLIENT_SECRET = '0uTUVrUgPWYqzOiHd7XZLWYTP63Jl9v2';
...
const guildSchema = new Schema({ guild: String, joins: Array, paramedicjoins: Array, joinChannels: Object, paramedicListMessage: String }, { strict: false });
...
app.get('/api/me', async (req, res) => {
    ...
    try {
        ...
        if (character.id?.job !== 'paramedic') return res.json({ loggedIn: true, registered: true, authorized: false, username: req.session.username, avatar: req.session.avatar });

        const onDuty = !!db?.paramedicjoins?.find(j => j.user === req.session.userId);
        ...
        return res.json({
            ...
            character: {
                ...
                rank: character?.id?.paramedic_data?.rank || '',
                sector: character?.id?.paramedic_data?.sector || '',
                code: character?.id?.paramedic_data?.code || '',
...
app.post('/api/duty/login', async (req, res) => {
    ...
    try {
        ...
        if (character.id?.job !== 'paramedic') return res.json({ success: false, message: 'هذا النظام للإسعاف فقط' });
        ...
        if (db.paramedicjoins?.find(j => j.user === req.session.userId)) return res.json({ success: false, message: 'أنت مباشر بالفعل' });

        if (!db.paramedicjoins) db.paramedicjoins = [];
        db.paramedicjoins.push({ user: req.session.userId, name: character.id.first, character: check.character });

        const pointsIndex = character.paramedic_points?.findIndex(p => p.name === 'login');
        if (pointsIndex !== -1 && character.paramedic_points) {
            character.paramedic_points[pointsIndex].value = (character.paramedic_points[pointsIndex].value || 0) + 4;
            userDoc.markModified(`characters.${check.character}.paramedic_points`);
            await userDoc.save();
        }

        db.markModified('paramedicjoins');
        await db.save();
...
app.post('/api/duty/logout', async (req, res) => {
    ...
    try {
        const db = await guildBase.findOne({ guild: GUILD_ID });
        const index = db?.paramedicjoins?.findIndex(j => j.user === req.session.userId);
        if (index === -1 || index === undefined) return res.json({ success: false, message: 'أنت لست مباشراً' });

        db.paramedicjoins.splice(index, 1);
        db.markModified('paramedicjoins');
        await db.save();
...
app.get("/api/mdt/:idNumber", async (req, res) => {
    ...
    try {
        ...
        if (!myChar || myChar.id?.job !== "paramedic") return res.json({ success: false, message: "للإسعاف فقط" });
        if (!db.paramedicjoins?.find(j => j.user === req.session.userId)) return res.json({ success: false, message: "يجب تسجيل المباشرة أولاً" });
...
// إذا شغّلنا هذا الملف مباشرة (زي "node server.js" على Render)، نشغّل الموقع
// فوراً (عشان Render يلقى port مفتوح بسرعة)، وبالتوازي نسجّل دخول البوت.
...
if (require.main === module) {
    // شغّل الموقع فوراً — ما ينتظر تسجيل دخول البوت، عشان Render يلقى
    // port مفتوح بسرعة ولا يفشل الـ deploy بسبب "no open ports detected".
    startWebsite();

    const { Client, GatewayIntentBits } = require('discord.js');

    if (!process.env.DISCORD_TOKEN) {
        console.error('❌ متغير البيئة DISCORD_TOKEN غير موجود. أضفه من Render > Environment. (الموقع شغّال، بس ميزات البوت معطّلة)');
    } else {
        ...
        botClient.login(process.env.DISCORD_TOKEN).catch(e => {
            console.error('❌ فشل تسجيل دخول البوت (الموقع شغّال بشكل مستقل):', e);
        });
    }
}
